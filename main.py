...
class GuiRoot(QWidget):
    ...
    def open_function(self):
        # open image function
        datapath = os.path.realpath(os.path.join(os.getcwd(), "ImageData")) # the default location
        ...
        if file_name[0]:
            ...
            if 'raw' in file_name[0]:
                ...
                self.img = numpy.stack((self.img,)*3, axis=-1) # stack gray scale img for rgb 3 channels
    ...
    def histogram_equation_function(self):
        def transform_function(cdf, height, width):
            ...
            cdf_min = numpy.amin(cdf) # get the minimun in cdf
            table = [0]*256 # table for 256 scale
            for i in range(256):
            # fill in the table with general histogram equalization formula
                table[i] = round(255 * (cdf[i] - cdf_min) 
                / (height * width -cdf_min), 0)
            
            for i in range(height):
            # look up the table for transition
    ...
    def GC_function(self):
        ...
        if self.original_flag:
            a = float(self.GC_degree.value()) # get Gamma value
            height, width, channel= self.img.shape 
            table =  [ [0]*256 for i in range(3)] #create table which could accerlate the computation

            for channel in range(3): 
                max_value = numpy.amax(self.img[:, :, channel])
                for index in range(256):
                    table[channel][index] = numpy.power(index / max_value, a) * max_value
                    # calculate correspounding gamma correction value for the 0 ~ 255 shades
